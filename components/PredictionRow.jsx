...
import { upsertPrediction, deletePrediction } from "@/lib/supabase"
...
export default function PredictionRow({ match, prediction, userId, onSaved, lockMinutes = 30 }) {
  ...
  const save = useCallback(async (h, a) => {
    ...
    if (isNaN(home) || isNaN(away) || home < 0 || away < 0 || home > 20 || away > 20) {
      setError("Bitte gültige Zahlen (0–20) eingeben")
      return
    }
    setSaving(true)
    setError("")
  ...
  }, [locked, match.id, userId, onSaved])

  function handleBlur() {
    if (homeTip !== "" && awayTip !== "") {
      save(homeTip, awayTip)
    }
  }
    ...
    <div className="bg-white rounded-2xl border border-orendt-gray-200 p-4 sm:p-5 hover:border-orendt-gray-300 transition-colors">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 sm:gap-4">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          {saving && <span className="text-[10px] text-orendt-gray-400">Speichern…</span>}
        </div>
