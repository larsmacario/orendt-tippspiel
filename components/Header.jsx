...
export default function Header({ user, currentPage = "dashboard" }) {
  ...
  if (user?.role === "admin") {
    tabs.push({ id: "admin", label: "Admin", icon: icons.settings, href: "/admin" })
  }
        ...
        <div className="flex items-center justify-between h-20">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="hidden sm:block">
              <span className="font-display text-xs sm:text-sm font-bold text-orendt-black uppercase tracking-tight">WM Tipprunde</span>
            </div>
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-1.5 sm:py-2 bg-orendt-gray-50 border border-orendt-gray-200 rounded-xl">
              ...
              <div className="hidden sm:flex flex-col">
                <span className="text-xs font-bold font-display text-orendt-black leading-none">{user?.display_name}</span>
...
function NavTab({ label, icon, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`
        flex items-center gap-2 px-4 sm:px-5 py-3 text-[12px] sm:text-[13px] font-display font-bold uppercase tracking-wider transition-colors border-b-2 whitespace-nowrap
        ${active
