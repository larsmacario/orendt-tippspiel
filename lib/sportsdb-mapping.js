...
/** strStatus „PEN“ o. ä. – Elfmeterschießen nach Unentschieden. */
export function isPenaltyShootoutStatus(raw) {
  const s = (raw || "").toLowerCase()
  return s === "pen" || s.includes("penalties") || s.includes("pen shoot")
}

/**
 * 90-Min.-Ergebnis für Tipps; bei PEN zusätzlich intHomeScoreExtra/intAwayScoreExtra.
 * @see lookupevent 2502846 (Germany vs Paraguay: 1–1, PEN 3–4)
 */
export function parseEventScores(event) {
  ...
  const isPen = isPenaltyShootoutStatus(rawStatus)
