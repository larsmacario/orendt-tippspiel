...
export const SUMMARY_LEGEND = [
  { key: "P", label: "Punkte in den 5 angezeigten Spielen", accent: true },
  { key: "B", label: "WM-Bonus" },
  { key: "S", label: "Schnitt pro getipptem Spiel" },
  { key: "G", label: "Gesamtpunkte", bold: true },
]

export function formatSnapshotMatchdayLabel(matchdayKey) {
  if (!matchdayKey) return null
  const [year, month, day] = matchdayKey.split("-")
  if (!year || !month || !day) return matchdayKey
  return `Spieltag ${day}.${month}.`
}

export function formatRankDeltaCaption(matchdayKey) {
  const label = formatSnapshotMatchdayLabel(matchdayKey)
  if (!label) return "+/- letzter Spieltag"
  return `+/- ${label}`
}

export function formatRankDeltaHint(matchdayKey) {
  const label = formatSnapshotMatchdayLabel(matchdayKey)
  if (!label) {
    return "Platzänderung im letzten abgeschlossenen Spieltag — Vergleich zum Stand davor, unabhängig von den 5 Spielspalten."
  }
  return `Platzänderung im ${label} — Vergleich zum Stand vor diesem Spieltag. Nicht aus Gesamtpunkte minus P ableitbar.`
}
