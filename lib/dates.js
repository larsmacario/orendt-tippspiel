...
/** Relatives Kickoff-Label für K.o.-Bracket-Karten (Gestern / Heute / Morgen). */
export function formatKickoffBracket(isoString) {
  if (!isoString) return "–"
  const date = new Date(isoString)
  const matchDay = getMatchdayKey(isoString)
  ...
  if (matchDay === todayKey) return `Heute ${time}`

  const yesterday = new Date()
  yesterday.setDate(yesterday.getDate() - 1)
  if (matchDay === getBerlinTodayKey(yesterday)) return "Gestern"

  const tomorrow = new Date()
  tomorrow.setDate(tomorrow.getDate() + 1)
  if (matchDay === getBerlinTodayKey(tomorrow)) return `Morgen ${time}`
