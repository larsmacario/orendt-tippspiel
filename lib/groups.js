...
export function getTeamFlagEmoji(teamName) {
  ...
  if (TEAM_FLAG_EMOJI[teamName]) return TEAM_FLAG_EMOJI[teamName]
  const key = Object.keys(TEAM_FLAG_EMOJI).find(
    (k) => k.toLowerCase() === teamName.toLowerCase() || teamName.toLowerCase().includes(k.toLowerCase())
  )
  return key ? TEAM_FLAG_EMOJI[key] : null
}
