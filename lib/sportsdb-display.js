import { mapStatus, mapPhase, parseScore, extractGroupCode } from "./sportsdb-mapping"
...
export function mapEvent(raw) {
  ...
  return {
    ...
    phase: mapPhase(raw.strRound, raw.strGroup),
    groupCode: resolveGroupCode(raw.strGroup, raw.strRound, homeTeam, awayTeam),
  }
}
...
export async function fetchScreenData() {
  ...
  const allSeasonMatches = seasonSchedule.map(mapEvent).filter(Boolean)

  const live = liveRaw.map(mapEvent).filter(Boolean)
