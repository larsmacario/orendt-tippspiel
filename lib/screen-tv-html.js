...
import { formatTeamCode, SUMMARY_LEGEND, SCREEN_TOP_LIMIT } from "./leaderboard-matrix"
...
function renderLeaderboardSlide(kicktipp) {
  ...
  if (!matches.length || !rows.length) {
    return `<div class="tv-slide-inner tv-slide-empty">
      <div class="tv-slide-head">
        <h2 class="tv-slide-title">Top 15 Tipprunde</h2>
  ...
  const meta = kicktipp.snapshotMatchday
    ? `<span class="tv-slide-meta">+/- seit ${escapeHtml(kicktipp.snapshotMatchday)}</span>`
    : ""
