...
function renderKnockoutTeamRow(name, badgeUrl, flagEmoji, score, isWinner, isLive) {
  ...
  return `<div class="tv-ko-row${winnerClass}">
    ${renderBadge(badgeUrl, name, flagEmoji, 22)}
    <span class="tv-ko-team" title="${escapeAttr(name)}">${escapeHtml(name)}</span>
    <span class="tv-ko-score${liveClass}">${score ?? "–"}</span>
  </div>`
}

function renderKnockoutSlot(match, slot) {
  const winner = getMatchWinner(match)
  const isLive = match.status === "live"
  const homeFlag = getTeamFlagEmoji(match.homeTeam)
  const awayFlag = getTeamFlagEmoji(match.awayTeam)
  const homeScore = match.status === "finished" || isLive ? match.homeScore : null
  const awayScore = match.status === "finished" || isLive ? match.awayScore : null
  const liveClass = isLive ? " tv-ko-slot-live" : ""

  return `<div class="tv-ko-slot${liveClass}">
    ${renderKnockoutTeamRow(match.homeTeam, match.homeBadge, homeFlag, homeScore, winner === "home", isLive)}
    ${renderKnockoutTeamRow(match.awayTeam, match.awayBadge, awayFlag, awayScore, winner === "away", isLive)}
  </div>`
}
...
function renderKnockoutSlide(knockout) {
  ...
  const bracket = buildKnockoutBracket(knockout)
  ...
  const connectorSvg = paths.length
    ? `<svg class="tv-ko-connectors" width="${totalWidth}" height="${totalHeight}" aria-hidden="true">
        ${paths
          .map(
            (p) =>
              `<path d="M ${p.x1} ${p.y1} H ${p.x2} V ${p.y3} H ${p.x4}" fill="none" stroke="#333" stroke-width="1.5" />`
          )
          .join("")}
      </svg>`
    : ""
      ...
      const slots = round.slots
        .map((slot) => {
          ...
          return `<div class="tv-ko-slot-wrap" style="top:${top}px;height:${height}px">
            ${renderKnockoutSlot(slot.match.raw || slot.match, slot)}
          </div>`
        })
        .join("")
  ...
  const final3Col = bracket.final3
    ? `<div class="tv-ko-round tv-ko-round-final3" style="width:${columnWidth * 0.7}px;height:${totalHeight}px">
        ...
        <div class="tv-ko-slot-wrap" style="top:${bracket.final3.rowStart * rowHeight}px">
          ${renderKnockoutSlot(bracket.final3.match.raw || bracket.final3.match, bracket.final3)}
        </div>
      </div>`
    : ""
