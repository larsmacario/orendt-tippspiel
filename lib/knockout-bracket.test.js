...
describe("buildVisibleKnockoutBracket", () => {
  ...
  it("marks projected slots as not tippable", () => {
    const bracket = buildVisibleKnockoutBracket(makeR32Matches())
    expect(isProjectedKnockoutMatch(bracket.rounds[1].slots[0].match)).toBe(true)
    expect(isProjectedKnockoutMatch(bracket.rounds[2].slots[0].match)).toBe(true)
    expect(isProjectedKnockoutMatch(bracket.rounds[4].slots[0].match)).toBe(true)
  })
})
