...
describe("calcPoints", () => {
  ...
  it("awards 2 points for draw tendency without goal diff points", () => {
    expect(calcPoints(1, 1, 2, 2)).toBe(2)
    expect(calcPoints(0, 0, 1, 1)).toBe(2)
  })
  ...
  it("uses the penalty winner for tendency after a 90-minute draw", () => {
    expect(calcPoints(1, 2, 1, 1, 3, 4)).toBe(2)
    expect(calcPoints(2, 1, 1, 1, 3, 4)).toBe(0)
    expect(calcPoints(0, 0, 1, 1, 3, 4)).toBe(0)
    expect(calcPoints(1, 1, 1, 1, 3, 4)).toBe(4)
  })
